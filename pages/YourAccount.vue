...
async function updatePhone(number: string) {
	...
	try {
		...
		currentPhone = await signInWithPhoneNumber(
			auth,
			number,
			applicationVerifier
		);
...
async function verifyCode(verificationCode: any) {
	...
	if (!newNumber.value) {
		await currentPhone.confirm(verificationCode);
...
async function verifyPhone(number: any) {
...
<template>
	...
	<div v-if="changePhone" class="modal-backdrop" style="">
		<form @submit.prevent="verifyPhone(newNumber)" action="">
			<div class="modal">
