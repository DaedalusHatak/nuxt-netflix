<script setup lang="ts">
  ...
  const logo: string = "daedalus.png";
  //screen and scroll
  ...
  const timerProfile = ref<NodeJS.Timeout>();
  //List refs
  ...
  //Hover profile functions
  function hoverProfile() {
    clearTimeout(timerProfile.value);
    if (!isNavigating.value) {
  ...
  function PreviousArrowKey($event: any) {
    ...
    if (isHoveredMenu.value === true) {
      ...
      const children = dropdown.value?.children[
        profileCounter.value
...
<template>
  <nav :style="{ backgroundColor: scroll ? '#331abbe6' : 'transparent' }">
    <div class="navbar">
      ...
      >
        ...
        >
          <NuxtLink
            ...
            @keydown.tab.exact.prevent="NextArrowKey"
            to="/browse"
            >Main Page</NuxtLink
          >
          <NuxtLink
            @keydown.shift.tab.exact.prevent="PreviousArrowKey"
      ...
      <div
        @keydown.enter.exact="
          isHoveredProfile ? unHoverProfile() : hoverProfile()
        "
    ...
    >
      ...
      >
        <NuxtLink
          ref="account"
          @keydown.shift.tab.exact="unHoverProfile()"
          @keydown.down.exact.prevent="NextArrowKey"
          @keydown.tab.exact.prevent="NextArrowKey"
          to="/YourAccount"
          >Account</NuxtLink
