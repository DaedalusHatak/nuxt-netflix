<script setup lang="ts">
  ...
  //Checks if any new posts are added
    ...
    doc.docChanges({}).forEach(async (snapshot) => {
      ...
      if (snapshot.type === "modified") {
        if (!isScrolling.value) {
          firestoreDatabase.value[
            firestoreDatabase.value.length - 1
          ].createdAt = snapshot.doc.data().createdAt;
        } else {
  ...
  //pushes new elements to array visible by user
